...
onMounted(async () => {
  ...
  readNext().then(() => {
    if (examQuestion.value.status === 'complete') navigateTo('/result');
  ...
  <Flex column gap="6" class="p-8" v-if="examQuestion">
    ...
    <Flex column items="center" justify="center" gap="2">
      ...
      <Flex column items="center" class="w-1/4" gap="2">
        <UButton block @click="isLoading[index] = true; submitQuestion(index).then(() => readNext().then(() => isLoading = [false, false, false, false]))" class="text-center" :loading="isLoading[index]" v-for="(i, index) in examQuestion.payload.options">{{ i }}</UButton>
      </Flex>
    </Flex>
